...
export default function SignupForm({ className = "" }: SignupFormProps) {
  ...
  return (
    <FormProvider {...methods}>
      <form onSubmit={methods.handleSubmit(onSubmit)} className={`w-full max-w-md ${className}`}>
        <motion.div
        ...
        >
          ...
          {currentStep < steps.length && (
            <div className="flex justify-between mt-6">
              ...
              <button
                type="button"
                onClick={currentStep === steps.length - 1 ? methods.handleSubmit(onSubmit) : nextStep}
              ...
              >
                {currentStep === steps.length - 1 ? "Create Account & Start Listing" : "Next"}
              </button>
            </div>
          )}
        </motion.div>
